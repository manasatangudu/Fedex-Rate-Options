...
export default class FedexRateExtractor extends LightningElement {
    ...
    getRates(){
       ...
       this.shipper= this.template.querySelector("c-address-input[data-my-id=shipper]").address;
       this.recipient= this.template.querySelector("c-address-input[data-my-id=recipient]").address; 
        getRateOptions({recipient : JSON.stringify(this.recipient) , shipper : JSON.stringify(this.shipper) , weightValue : this.weightValue})
        .then(result =>{
            if(result){
